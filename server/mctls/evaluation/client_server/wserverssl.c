...
int serveFile(SSL *ssl, int s, char *filename, char *proto){ 
	...
	int i = fread(buf, file_size, 1, fp);    //lee los datos de fichero fp, buf=puntero de memoria,file_size=tamaño
	                                         ...
	                                         //Puntero a objetos FILE, que especifica la cadena de entrada.
...
static int http_serve_request(SSL *ssl, int s, char *proto, bool shut, int action){
	...
	int fSize = atoi(filename);  //convirte un argumento string en un entero (int), cuando el valor
	                             //sean letras entonces retorna cero, quiere decir que la conversion no se pudo realizar
