...
static int http_serve_request(SSL *ssl, int s, char *proto, bool shut, int action, SSL_CTX *ssl_ctx){
	...
	while(1){
		...
		sscanf(buf, "GET /%99s HTTP/1.1\r\n Host: %99[^:]:%99d\r\n\r\n", page, ip, &port);

		#ifdef DEBUG
		printf("[DEBUG] IP SERVER WEB %s:%d\n", ip,port);
		#endif
		
		//AÑADIENDO CONVERSIÓN SPP A SSL
		sock_new = tcp_connect(ip,port);
...
int main(int argc, char **argv){
	...
	FILE *fp;
	pf = freopen("wserverLogs.txt","+w",stdout);
	printf("Output redirected to wserverLogs.txt\n");
	...
	while(1){
		...
		if (pid == 0){
			...
			switch(action){
				...
				// Serve some content 
				case 3: http_serve_request(ssl, newsock, proto, false, action, ssl_ctx);
						break;
