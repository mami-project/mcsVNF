...
int calculate_file_size(char *filename){ 
    ...
    fseek(fp, 0L, SEEK_END);  //establece la posicion file del fp (lectura de filname) al final del fichero
    sz = ftell(fp);     //retorna la posicion actual del fichero, en este caso es el final del fichero
                        //(SEEK_END)
...
int read_proxy_count(char *file_name){
	...
	fp = fopen(file_name,"r");	//abre y lee el file proxy_listoriginal
   
	// CHheck for errors while opening file
	if( fp == NULL ){
		printf("Error while opening file %s.\r\n", file_name);
		exit(-1);  //nreak el programa
    }

	// Read first line of file
	fgets ( line, sizeof line, fp );   //lee cadena de caracteres, en este caso hasta que haya un \n
	                                   //fgets(buffer,tamaño del buffer, fichero)
	
	// Convert char into integer 
	N = atoi(line);   //atoi convierte string a int para imprimir en screen
...
void read_proxy_list(char *file_name, SPP_PROXY **proxies){
	...
	while ( fgets ( line, sizeof line, fp ) != NULL ) {  //lee cadenas de caracteres, que para este caso
		                                                 //lee hasta que haya un \n. 
		                                                 //fgets(buffer,tamaño del buffer, fichero)
	
		// Remove trailing newline (NOTE: strtoK is not thread safe)
		strtok(line, "\n");   //strtok es un delimitador, aca dice que lee la lineas de proxy_lisoriginal
		                      //hasta que haya un \n
		...
		strcpy(newLine, line);   //copia contenido de line en newLine, strcpy(destino, fuente)
...
int tcp_connect(char *host, int port){
	...
	// Resolve host 
	if(!(hp = gethostbyname(host))){    //retorna en hostent la info que contiene host, como hostname, list aliases ip
		                                ////longitud de direcciones etc.
...
void check_cert(char *host){
	...
	if ((res == X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT) || (res == X509_V_ERR_SELF_SIGNED_CERT_IN_CHAIN)){
		// || es un operador logico OR que dice si ref = a .. y tambien es == a ..
		/*
		#ifdef DEBUG
...
// Form and send GET
void sendRequest(char *filename, char *url){
	...
	// Form the request 
	memset(request, '0', sizeof(request)); //memset copia 0 en request, la cantidad de 
	                                        //ceros que copie dependerá del size de request
	sprintf(request, "GET /hello.html HTTP/1.1\r\nHost: 192.168.122.7:443\r\n\r\n"); 
	//opne Get HTTP/1.1 .. en request
	request_len = strlen(request);  //strlen devuelve la longitud de request
...
static int http_request(bool requestingFile, char *url){
	char buf[BUFSIZZ];
	int r;
	int len; 
	long bytes_read = 0;
	...
	*/
	...
	memset(request, '0', sizeof(request)); //memset copia 0 en request, la cantidad de 
	                                        //ceros que copie dependerá del size de request
	sprintf(&request,"GET /%s %sHost: %s:%d\r\n\r\n",page,standar,ip,port);
	//sprintf(request, "GET /hello.html HTTP/1.1\r\nHost: 192.168.122.7:443\r\n\r\n");  
	//sprintf(request, "GET /hello.html HTTP/1.1\r\nHost: %s:%d\r\n\r\n", host, port); 
	//opne Get HTTP/1.1 .. en request
	request_len = strlen(request);  //strlen devuelve la longitud de request
	...
	/*
	#ifdef DEBUG
	printf("[DEBUG] REQUEST LOOK LIKE AS: \n%s\n", request);
	#endif	
	*/
	// SPP write
	if (strcmp(proto, "spp") == 0){
	...
	while(1){
		...
		if (strcmp(proto, "spp") == 0){
			...
			*/
			...
			printf("[INFO] WAITING ON SPP_READ...\n");
			#endif			
			*/
			SPP_SLICE *slice;		// slice for SPP_read
			SPP_CTX *ctx;			// context pointer for SPP_read
...
void usage(void){
	...
	printf("-url: introduce URL path. eg: -url http://localhost.com/\n");
	exit(-1);  
}
...
int main(int argc, char **argv){
	...
	char *filename = "proxyList_original";          // filename for proxy
	...
	FILE *fp;
	printf("All text sent to stdout\n");
	fp = freopen("fileLogs.txt","w+",stdout);
	////////
	char *url = NULL;
...
*/
	...
	while((c = getopt(argc, argv, "s:r:w:i:f:c:o:a:b:u:")) != -1){
			
			switch(c){
	
			// Number of slices
			case 's':	if(! (slices_len = atoi(optarg) )){ //optarg son los parametros de entrada del ususario
				                                             //atoi me convierte de un string a un entero (int)
							err_exit("Bogus number of slices specified (no. of slices need to be > 1)");
						}
						experiment_info->num_slices = slices_len; //la variable num_slices (que esta declarada en
																//la struct experiment_info del common.h) tomara
																//el valor de slices_len
						//las structs sirven para almacenar varios tipo de datos en diferentes variables
						break;
			...
			case 'f':	if(! (file_requested = strdup(optarg) )){  //file_request es el index.html
							err_exit("Out of memory");
						}
						break; 
	...
	experiment_info->num_proxies = N_proxies - 1;  //experiment_info es una struct creada en common.h
												   //num_proxies es un int creado a partir de esa struct
	
	// Check that input parameters are correct 
	/*
	#ifdef DEBUG
	printf("[DEBUG] Parameters count: %d\n", argc);   //argc, valor del  numero de opciones o flags (s:r:w:i:f:c:o:a:b:)
	#endif
	*/
	if (argc == 1){
		usage(); 
	...
	if (r > N_proxies || w > N_proxies){   //r=numero de proxies de lectura, w=numero de proxies de escritura
		printf ("The values for r and w need to be <= than the number of proxies\n"); 
		usage(); 
	}
	if (url == NULL){
		#ifdef DEBUG
		printf("SPECIFICATE URL PATH\n");
		#endif
		usage();
	}
	else{
		/*
		#ifdef DEBUG
		printf("[DEBUG] SPECIFICATED URL %s\n",url);
		#endif
		*/
	}
	// Generate a clientID
	...
	*/
	...
	// Build SSL context
	ctx = initialize_ctx(KEYFILE, PASSWORD, proto);  //usa el metodo SPP_method()
	ssl = SSL_new(ctx); //crea una nueva estructura ssl para una conexion

	// Allocate memory for proxies 	
	proxies  = malloc( N_proxies * sizeof (SPP_PROXY*));  //guardar variables proxies en memoria
	                                                      //retorna un puntero o valor o NULL si la request falla
	...
	*/
	...
	for (i = 0;  i < slices_len; i++){
		char *newPurpose;  
		char str[30]; 
		sprintf (str, "slices_%d", (i + 2));  //pone en str slice_2, slice_3... dependiendo de la cant. de slices
		newPurpose = (char *)malloc(strlen(str)+1);    
		strcpy(newPurpose, str);  //copia el contenido de str a newPurpose
	...
	int MAX = r; 
	if (w > r)   //r=numero de proxies de lectura, w=numero de proxies de escritura
		MAX = w; 
	...
	memcpy(address, proxies[0]->address, strlen(proxies[0]->address) + 1); //memcpy copia en memoria
	                                      //lo que haya en le posicin 0 de proxies->address en address
	                                      //memcpy(dst,src,strlen(src)+1)
	host = strtok(address, ":");   //me devulve el valor de address hasta el :
	port = atoi(strtok(NULL, ":")); //atoi me convierte de string a entero (int), ademas
	                                ///me duelve el valor de port hasta :, esto lo hace de atras para adelante
	/*
	#ifdef DEBUG 
	...
	*/
	...
	if (strcmp(proto, "pln") != 0) 
	{
		sbio = BIO_new_socket(sock, BIO_NOCLOSE);  //crea un socket BIO TCP/IP para SSL, sock es el socket creado
		                                           //con tcp_connect()
	...
	switch(action){
		...
		case 2:		http_request(false, url);
					break; 

		// Send HTTP GET request and wait for file to be received
		case 3:		http_request(true, url);
					break; 
	...
	*/
	...
	/*
	#ifdef DEBUG
	printf("ENTRA EN SEGUNDO FREE PROXYS\n");
	#endif
	free(proxies); 
	*/
	free(slice_set);
