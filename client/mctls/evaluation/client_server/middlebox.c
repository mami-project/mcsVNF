...
SSL* SPP_Callback(SSL *ssl, char *address){
        ...
        return create_SSL_connection(address, "middlebox");  //al final me va a retornar new_ssl, es decir
                                                             //nueva estructura ssl
    ...
    int r,w;
        ...
        while(1){
                ...
                fwrite(buf, 1, r, stdout);  ////escribiendo; buf=contenido de info, 1=tamaño de cada registro
                                             //r numero de elementos, donde quiere que se escriba la informacion
                printf("\n******\n");
...
int handle_data(SSL* prev_ssl, SSL* next_ssl, char* proto)
{
        ...
        next_handler = fork();  //crea un proceso que si retorna valor negativo el proceso no fue exitoso
        if( next_handler == 0)
        {
                //child process
...
int main(int argc, char **argv){
	...
	//Added to keep the info in a file
	FILE *fp;
	fp = freopen("middleboxLogs.txt","w+", stdout);
	printf("Output is being redirected to middleboxLogs.txt\n");
