...
int tcp_listen(int port)
  {
    ...
    memset(&sin,0,sizeof(sin));       //memset copia 0 en sin, lo copia segun sea el size de sin
    sin.sin_addr.s_addr=INADDR_ANY;    //pone la Ip del seridor automáticamente
    sin.sin_family=AF_INET;            //familia de la dirección 
    sin.sin_port=htons(port);          //escoge el numero de puerto
    setsockopt(sock,SOL_SOCKET,SO_REUSEADDR,  //setsockopt es exitoso retorna 0, sino retorna -1
      &val,sizeof(val));

    if (disable_nagle == 1)
    	set_nagle(sock, 1);        //se deshabilita el algoritmo nagle's y hace que se puedan enviar
    	                           //datos pequeños tcp ej: 1 byte de datos
    
    if(bind(sock,(struct sockaddr *)&sin,   //si el socket retorna -1 es que dio error
      sizeof(sin))<0)
      berr_exit("Couldn't bind");
    listen(sock,5);     //se usa si se están esperando conexiones entrantes
                        //o cual significa, si se quiere, que alguien pueda conectarse a nuestra máquina
                         //después de llamar a listen(), se deberá llamar a accept()
                         //para así aceptar las conexiones entrantes
...
int tcp_connect(char *host, int port){
	...
	struct hostent *hp;   //servidor
	struct sockaddr_in addr;  //cliente
	int sock;

	// Resolve host 
	if(!(hp = gethostbyname(host))){          //retorna en hostent info que contiene host, como hostname, list aliases ip
		berr_exit("Couldn't resolve host");  //longitud de direcciones etc.
	}
	#ifdef DEBUG
...
void load_dh_params(ctx,file)
  ...
  char *file;
  {
    ...
    if ((bio=BIO_new_file(file,"r")) == NULL)   //lee los datos "r"
...
SSL* create_SSL_connection(char *address, char* method){
	...
	printf(address);
	#endif
	char* ipv4 = strtok(strdup(address), ":");	// ip   //me devulve el valor de address hasta el :
    int port = atoi(strtok(NULL, ":"));	// port //atoi me convierte de string a entero (int), ademas
	...
	ctx = initialize_ctx(KEYFILE, PASSWORD, method);  //inicializa nuevo contexto
	new_ssl = SSL_new(ctx);    //crea nueva estructura ssl para conexion
	sock = tcp_connect(ipv4, port);  //conecta socket con ip y puerto del servidor
	sbio = BIO_new_socket(sock, BIO_NOCLOSE); //crea un socket BIO TCP/IP.
    SSL_set_bio(new_ssl, sbio, sbio);   //al nuevo contexto le va a permitir leer y escribir la conexion al server 
...
{  
	...
	while(1){
		...
		#ifdef DEBUG
		printf("[middlebox-p] Data received (from previous hop) (length %d bytes):\n*****\n", r); //r=tamaño en bytes de
		 //lo que me devuelve SPP_read_record()
		fwrite(buf, 1, r, stdout);    //escribiendo; buf=contenido de info, 1=tamaño de cada registro
		                              //r=el numero de registros a escribir, stoud(salida estandar
		                              //de unix)=la salida donde se va a escribir
		                              //lo que esta en buf 
		printf("\n******\n"); 
...
{  
	...
	while(1){
		...
		{
			r = SSL_read(next_ssl, buf, BUFSIZZ);  //intenta leer el numero de bytes (BUFSIZZ) a partir de la next_ssl
			                                       //dentro del buffer(buf)
		}
...
{
	pid_t next_handler;   //representa el PID del proceso
	int status;
	...
	next_handler = fork();  //crea un proceso y lo llama next_handler, y como no es cero entra la next_hop
	if( next_handler == 0) 
	{
		//child process
...
int main(int argc, char **argv){
	int sock, s;
	BIO *sbio;
	SSL_CTX *ctx;         //varible de c para crear cliente ?? o server ssl
	SSL *ssl;              //varible de c para crear cliente ?? o server ssl
	int r;
	pid_t pid;
	...
	if (strcmp(proto, "fwd") != 0){    //strcmp compara dos cadenas, si devuelve un 0, quiere decir que son iguales
		                                //si devuelve mayor que 0 quiere decir que proto es mayor, si devuelve menos de
		                                 //cero, quiere decir que fwd es mayor que proto
		if (strcmp(proto, "spp") == 0)
			ctx = initialize_ctx(KEYFILE, PASSWORD, "middlebox");   //va al common.h (cabeceras de las funciones y tipos de datos)
		                                                            //y saca initialize_ctx, y el common.h es llamado por
		                                                            //el common.c
		else 
	...
	while(1){
		...
		if((s = accept(sock, 0, 0)) < 0){    //tcp_listen retorna sock, el accept esta ligado a conexiones entrantes
			err_exit("Problem socket accept\n"); //si sock es -1 es que hay un error
		}
		signal(SIGCHLD, SIG_IGN);   //esto es para el timer de tiempo cpu
		// fork a new proces 
		if((pid = fork())){
			close(s);
		} else {
			...
			if (strcmp(proto, "fwd") == 0){
				...
				sbio = BIO_new_socket(s, BIO_NOCLOSE);  //crea un socket BIO TCP/IP, s es el socket creado con tcp_listen() 
